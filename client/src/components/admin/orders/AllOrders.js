...
import { UPDATE_ORDER_MODAL_OPEN } from '../../../constants/orders'
...
const OrderTable = ({ order }) => {
    ...
    const deleteOrderReq = (oId) => {
        dispatch(deleteOrder(oId))
    }

    const editOrder = (oId, status) => {
        dispatch({ type: UPDATE_ORDER_MODAL_OPEN, oId, status })
    }
        ...
        <Fragment>
            <tr className="border-b">
                ...
                <td className="p-2 flex items-center justify-center">
                    <span
                        onClick={() => editOrder(order._id, order.status)}
        ...
        <Fragment>
            <div className="col-span-1 overflow-auto bg-white shadow-lg p-4">
                <table className="table-auto border w-full my-2">
                    ...
                    <tbody>
                        {data.ordersInstance.length ? data.ordersInstance.map((item, i) => (
                            <OrderTable
                                key={i}
                                order={item}
                            />
                ...
                <div className="text-sm text-gray-600 mt-2">
                    Total {data.ordersInstance.length || 0} order found
        </div>
      </div>
    </Fragment>
    )
