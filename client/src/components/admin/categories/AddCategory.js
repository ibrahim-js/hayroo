...
import { addCatModal, createCategory } from '../../../actions/category'

const AddCategory = () => {
    const dispatch = useDispatch()
    const data = useSelector(state => state.category)

    const [fData, setFdata] = useState({ cName: "", cDescription: "", cImage: "", cStatus: "Active", success: null, error: null })

    if (fData.success || fData.error) {
        setTimeout(() => {
            setFdata({ ...fData, success: null, error: null })
        }, 3000)
    }
    ...
    const submitForm = (e) => {
        ...
        dispatch(createCategory(fData, setFdata))
    }
    ...
    return (
        <Fragment>
            {/* Black Overlay */}
            <div
                onClick={() => dispatch(addCatModal(false))}
